...
model_type = "linear"
NFIT = 3
data_raw = 'planar_x1_x2_y.json'

# NFIT = 4
# data_raw = 'planar_x1_x2_x3_y.json'
...
X = np.column_stack((np.array(data['x1']), np.array(data['x2'])))
# X = np.column_stack((np.array(data['x1']), np.array(data['x2']), np.array(data['x3'])))
Y = np.array(data['y'])
...
#------------------------
def model(x,p):
	if(model_type=="linear"):  return p[0] + np.matmul(x, p[1:].reshape(NFIT-1, 1))
