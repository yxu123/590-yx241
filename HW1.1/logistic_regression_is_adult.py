...
def loss(p):
    ...
    training_loss = 0.5 * ((model(raw.train_x,p)-raw.train_is_adult)**2).sum()
    validation_loss = 0.5 * ((model(raw.validation_x,p)-raw.validation_is_adult)**2).sum()
    ...
    return 0.5 * ((model(raw.train_x,p)-raw.train_is_adult)**2).sum()
...
plt.plot(raw.train_x,raw.train_is_adult,'bo',label="train set")
plt.plot(raw.validation_x,raw.validation_is_adult,'gx',label="validation set")
plt.plot(raw.test_x,raw.test_is_adult,'r*',label="test set")
